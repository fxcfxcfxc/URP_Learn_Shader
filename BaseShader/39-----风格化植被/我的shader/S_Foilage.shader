Shader"Myshader/Foilage"
{
    Properties
    {  
        _DiffTexture("DiffTexture",2D)="black"{}
        _normalMap("normalMap",2D)="bump"{}
        _CullTexture("CullTexture",2D)="white"{}
        
        
        [Header(__________________________________Main Setting)]
        _aphlaCulloff("_aphlaCulloff",Range(0,1))=0.5
    ...
    {   
        ...
        Pass
        {
            ...
            TEXTURE2D(_CullTexture);
            SAMPLER(sampler_CullTexture);
            ...
            */
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float4 cullTexture = SAMPLE_TEXTURE2D(_CullTexture,sampler_CullTexture,uv0);
                
                //----------------------------------------------------计算

                float nDotl = saturate( dot(nDirWS, lDirWS) );
                
                clip(cullTexture.a- _aphlaCulloff);
                float3 fragementOutColor =lerp( _shadowColor.rgb,_MainColor * cullTexture.r  , nDotl) ;    
                return float4(fragementOutColor,1.0);
            }
