        ...
        Pass
        {
            ...
            */
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                //----------------------------------------------------计算
                //拿到方向光之外光源数量
                int addlight = GetAdditionalLightsCount();
                float4 finalColor = 0;
                
                for(int index = 0; index <addlight; index++)
                {
                    Light addl = GetAdditionalLight(index, posWS);
                    finalColor.rgb  += addl.color  * addl.distanceAttenuation * addl.shadowAttenuation;
                    
                }
                float3 nDotl = max(0, dot(nDirWS, lDirWS)) *  lightCol;
                
                float3 fragementOutColor = finalColor + nDotl;    
